...
void TramitaOrdre(void) {
	...
	if (Quants) {
		...
		if (t & ES_IR) {
			t &= 0x00ff;
			SetD4_D7Sortida(); RSDown(); 
			EnableUp();
			CantaPartAlta(t); 		// Segons hitachi Data Setup = 80ns (cap problema)
			EnableDown();   		// i l'amplada del pols "enable" major que 230n
			EnableUp();				
			CantaPartBaixa(t); 	// Segons hitachi Data Setup = 80ns (cap problema)
			EnableUp();				// Asseguro els 500ns de durada de pols
			EnableDown();   		// i l'amplada del pols "enable" major que 230n
			RSUp();
		} else {
...
void WaitForBusy(void) {
/*	char Busy; 
	// Amb l'expansor al mig, no semba que faci falta
	SetD4_D7Entrada(); 
	RSDown();
	RWUp();
	TiResetTics(Timer);
